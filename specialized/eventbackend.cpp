...
struct EventBackend::platform_dependant // poll notification (epoll)
{
  ...
  static constexpr uint8_t from_native_flags(const uint32_t flags) noexcept
  {
    return
        ...
        (flags & EPOLLHUP ? Event::Disconnected   : 0) |
        (flags & EPOLLIN  ? Event::Readable       : 0) |
  ...
  static constexpr uint32_t to_native_flags(const uint8_t flags) noexcept
  {
    return
        ...
        (flags & Event::Disconnected ? uint32_t(EPOLLHUP ) : 0) |
        (flags & Event::Readable     ? uint32_t(EPOLLIN  ) : 0) |
        (flags & Event::Writeable    ? uint32_t(EPOLLOUT ) : 0);
  }
  ...
  bool add(posix::fd_t wd, Event::Flags_t flags) noexcept
  {
  ...
  bool modify(posix::fd_t wd, Event::Flags_t flags) noexcept
  {
    struct epoll_event native_event;
    native_event.data.fd = wd;
    native_event.events = to_native_flags(flags); // be sure to convert to native events
...
bool EventBackend::poll(int timeout) noexcept
{
  ...
  for(epoll_event* pos = s_platform.output; pos != end; ++pos) // iterate through results
    results.emplace_back(std::make_pair(posix::fd_t(pos->data.fd), platform_dependant::from_native_flags(pos->events))); // save result (in non-native format)
  return true;
}
